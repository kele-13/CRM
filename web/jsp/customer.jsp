...
<body>
	...
	<div id="wrapper">
		...
		<ul class="nav navbar-top-links navbar-right">
			...
			<!-- /.dropdown -->
			<li class="dropdown"><a class="dropdown-toggle"
				data-toggle="dropdown" href="#"> ${sessionScope.user.userName}<i class="fa fa-user fa-fw"></i>
					<i class="fa fa-caret-down"></i>
			</a>
				<ul class="dropdown-menu dropdown-user">
					...
					<li class="divider"></li>
					<li><a href="${pageContext.servletContext.contextPath}/logoutuser"><i class="fa fa-sign-out fa-fw"></i>
							退出登录</a></li>
				</ul> <!-- /.dropdown-user --></li>
			<!-- /.dropdown -->
		</ul>
		...
		<div id="page-wrapper">
			...
			<div class="panel panel-default">
				<div class="panel-body">
					<form class="form-inline" action="${pageContext.request.contextPath }/customer/list" method="get" id="form">
						<div class="form-group">
							<label for="customerName">客户名称</label> 
							<input type="text" class="form-control" id="customerName" value="${searchInfo.custName }" name="custName">
						</div>
						<div class="form-group">
							...
							<select	class="form-control" id="customerFrom" placeholder="客户来源" name="custSource">
								...
								<c:forEach items="${fromType}" var="item">
									<option value="${item.dictId}"<c:if test="${item.dictId == searchInfo.custSource}"> selected</c:if>>${item.dictItemName }</option>
								</c:forEach>
							</select>
						</div>
						<div class="form-group">
							...
							<select	class="form-control" id="custIndustry"  name="custIndustry">
								...
								<c:forEach items="${industryType}" var="item">
									<option value="${item.dictId}"<c:if test="${item.dictId == searchInfo.custIndustry}"> selected</c:if>>${item.dictItemName }</option>
								</c:forEach>
							</select>
						</div>
						<div class="form-group">
							...
							<select	class="form-control" id="custLevel" name="custLevel">
								...
								<c:forEach items="${levelType}" var="item">
									<option value="${item.dictId}" <c:if test="${item.dictId == searchInfo.custLevel}"> selected</c:if> >${item.dictItemName }</option>
								</c:forEach>
							</select>
						</div>
						<label for="size">每页显示</label>
						<input style="width: 50px" id="size" name="size" value="${searchInfo.size}"/>条
						<input id="page" hidden="hidden" name="page" value="${searchInfo.page}"/>
						<button type="button" onclick="submitFM()" class="btn btn-primary">查询</button>
					</form>
				</div>
			</div>
			<div class="row">
...
<%--							<kkb:paging url="${pageContext.request.contextPath }/customer/list" />--%>
